...
npm `latest` and advance a stale `next` to the same stable version without replacing
a newer preview; GitHub prereleases publish to `next`. The pipeline never writes
back to `main`.
