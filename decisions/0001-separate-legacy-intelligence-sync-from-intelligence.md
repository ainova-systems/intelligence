# 0001 — Archive Intelligence Sync and separate the Intelligence CLI product
...
The CLI work additionally made the Intelligence Sync engine CLI-aware. Merging it into the legacy
product would leave the vendored lineage with Intelligence scaffolding grown into it — the wrong thing to archive.

## Decision

1. **The products split at `d124af6` (v0.10.1).** The Intelligence branch never merges into legacy Intelligence Sync.

2. **`ainova-systems/intelligence-sync` freezes in place as the legacy Intelligence Sync archive**, at the same URL its
...
3. **`ainova-systems/intelligence` is the Intelligence product**, seeded by pushing the CLI branch as `main`.
   History travels; **tags do not** — the resolver enumerates versions with `git ls-remote`, and
   legacy product tags point at trees where `packages/sync` does not exist. The Intelligence tag line starts at `v0.11.0`.
...
5. **Legacy Intelligence Sync machinery is deleted from the Intelligence engine**: `packs:`/mirrors/`.pack`, `update.sh`,
...
6. **Legacy Intelligence Sync conversion requires its final schema.** A project older than that runs its own
   `update.sh` first — possible forever, because the archive stays at the URL its frozen script
   hardcodes. This is what lets Intelligence drop `lib/migrations.sh`: no legacy chain crosses the product boundary.
...
8. **Meta-skills become interpreters, not implementations.** In legacy Intelligence Sync a skill
   carried the whole procedure because there was no program; Intelligence has one, so a skill carries only what a program
   cannot. The originally proposed command split was superseded by
   [decision 0002](0002-consolidate-intelligence-cli-lifecycle.md).
...
- The Intelligence repo must re-point: `npm/package.json` `repository`/`homepage`/`bugs` (npm publishes with
...
- **Rename `intelligence-sync` and reclaim the old name for the archive.** It would have kept the
  audience with Intelligence and redirected old clients for free, but namespace retirement past 100
...
- **A minimal placeholder on `main`.** Actively destructive: a frozen `update.sh` runs
  ...
  stubbing is not.
- **Recreating the Intelligence repo from scratch.** 571 KB and 56 commits; it would lose blame on the
