...
One CLI owns the whole lifecycle: a root `intelligence.yaml` manifest, versioned **Intelligence Packages** with an `intelligence.lock`, and per-tool output generated by the sync engine. Teammates restore everything after cloning with a single `intelligence install`. Any git repo with `rules/`, `agents/` or `skills/` at its root is already a package — install it from an explicit source (`intelligence add github:your-org/your-repo`), or by name once a registry you trust declares it (`intelligence registry add <registry-repo-url>`); versions come from its `x.y.z` git tags. See [docs/CLI.md](docs/CLI.md).

### Vendored setup (the original flow — still fully supported)
...
you never run a `git clone` or `cp` yourself. A vendored project converts to the CLI setup any
time with `intelligence migrate` (transactional, `--dry-run` available) — and keeps working as-is
if it never does.
