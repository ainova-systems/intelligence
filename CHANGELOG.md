...
- **`intelligence registry add` accepted a URL that is not a registry.** A registry is a git repo with `index.yaml` at its root; pointing a scope at a package repo instead bound silently and only failed at the first `add`. The bind now fetches the index immediately and reports how many packages it offers, or warns (still writing the binding — the registry may just not exist yet) with the format's documentation.
...
- **Engine `scripts/` are staged into `.intelligence/engine/`.** Engine-shipped skills reference `<module>/scripts/…` (adapter authoring, vendored docs) and `<module>` resolves into the store, so those paths must exist in a CLI project too.
