...
- `intelligence upgrade [--preview|--apply]` replaces the installed CLI with the newest version on its npm channel (`next` for a prerelease, otherwise `latest`) with the same modes. It touches no project and refuses an installation that npm did not make.
